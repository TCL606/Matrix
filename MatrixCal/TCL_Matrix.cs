    ...
    public class Matrix : ICloneable, IEnumerable
    {
        ...
        public bool InverseMatrix(out Matrix inverse)
        {
            ...
            try
            {
                ...
                Matrix caculateInverse = new(row, 2 * row);
                for (int i = 0; i < row; i++)
                    for (int j = 0; j < row; j++)
                    {
        ...
        public bool GetBasesOfNullSpace(out Matrix? bases)
        {
            try
            {
                ...
                if (ret.GetAllSolutionsForAugmentedMatrix(out ret))
                {
                    ...
                    Matrix nullSpace = new(ret.row, ret.col - 1);
        ...
        public bool SVD(out Matrix? U, out Matrix? Sigma, out Matrix? VT)
        {
            try
            {
                ...
                Matrix V = new(this.col, this.col);
                ...
                if (this.col< this.row)
                {
                    ...
                    int now = 0;
                    Matrix Sigmar = new(ATADim, ATADim);  //作一个Sigmar矩阵
                    for (int i = 0; i < len; i++)
                    {
                    ...
                    if (V1.col < V.col)
                    {
                        ...
                        Matrix zero = new(V.row, 1);
                        Matrix V2 = new(V.row, V.col -V1.col);
                    ...
                    if (U1.col < U.col)  //当原矩阵不是方阵时，这里的U1还不是U，要补充正交向量
                    {
                        ...
                        Matrix zero = new(U.row, 1);
                        Matrix U2 = new(U.row, U.col - U1.col);
                ...
                else
                {
                    ...
                    int now = 0;
                    Matrix Sigmar = new(AATDim, AATDim);  //作一个Sigmar矩阵
                    for (int i = 0; i < len; i++)
                    {
                        for (int j = 0; j < multiplicity[i]; j++)
                        {
        ...
        public static Matrix operator &(Matrix A, Matrix B)
        {
            try
            {
                ...
                {
                    Matrix ret = new(A.row, A.col +B.col);
